...
const searchForm = document.querySelector("form"); // Target the form
const input = document.querySelector('#search-bar');

// Listen to the form submit instead of just the button click
...
function search(event) {
    ...
    // output onto screen
    sortedRecipes.forEach(renderRecipe);
}
...
function difficultyTemplate(rating) {
    let html = `<span class="rating" role="img" aria-label="Rating: ${rating} out of 5">Rating: `;
    for (let i = 1; i <= 5; i++) {
        if (i <= Math.floor(rating)) {
            html += `<span aria-hidden="true" class="icon-star">⭐</span>`;
        } else {
            html += `<span aria-hidden="true" class="icon-blank-star">☆</span>`;
        }         
    }
    html += `</span>`;
    return html;
...
function recipesTemplate(recipe) {
    // Note: Changed recipe.difficulty to recipe.rating
